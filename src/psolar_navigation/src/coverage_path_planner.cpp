#include "psolar_navigation/coverage_path_planner.hpp"
...
std::list<Point_t> CoveragePlanner::sweep_by_tool_width(
  ...
  const geometry_msgs::msg::PoseStamped & goal)
{
  ...
  // Run the zigzag fill algorithm with tool width only
  double robot_diameter_m = 2.0 * robot_radius_;
  double tool_width_m = robot_diameter_m * 1.3;

  std::list<Point_t> pathPoints = sweep_by_tool_width(
...
PLUGINLIB_EXPORT_CLASS(psolar_navigation::CoveragePlanner, nav2_core::GlobalPlanner)
